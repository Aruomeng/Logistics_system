...
import { logisticsListOrders, logisticsUpdate, logisticsDelivered, getOrderDetail } from '../../api/order'
...
const handleUpdateTrack = async (row: any) => {
  const { value } = await ElMessageBox.prompt('请输入物流更新信息', '更新物流', { inputPlaceholder: '如：已到达XX中转站' })
...
const handleDelivered = async (row: any) => {
  await ElMessageBox.confirm('确认货物已送达？', '提示')
    ...
    <div class="card-content">
      ...
      <div class="search-bar">
        <el-select v-model="searchForm.status" placeholder="订单状态" clearable style="width: 140px;">
          <el-option label="运输中" :value="2" /><el-option label="待收货" :value="3" /><el-option label="已完成" :value="4" />
        </el-select>
      ...
      <el-table :data="tableData" v-loading="loading" stripe border>
        ...
        <el-table-column label="操作" width="240" fixed="right">
          <template #default="{ row }">
            ...
            <el-button v-if="row.orderStatus === 2" size="small" type="warning" link
